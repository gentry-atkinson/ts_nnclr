...
import lightly
from lightly_plus_time.lightly.api.version_checking import get_latest_version, \
...
class TestVersionChecking(unittest.TestCase):
    ...
    def test_version_check_timout_mocked(self):
        """
            ...
            Thus this only checks that the actual lightly_plus_time.lightly.do_version_check()
            with needing >1s internally causes a LightlyAPITimeoutException
        """
        try:
            old_get_versioning_api = lightly_plus_time.lightly.api.version_checking.get_versioning_api
            ...
            lightly_plus_time.lightly.api.version_checking.get_versioning_api = mocked_get_versioning_api_timeout

            start_time = time.time()

            with self.assertRaises(LightlyAPITimeoutException):
                do_version_check(lightly_plus_time.lightly.__version__)
            ...
            lightly_plus_time.lightly.api.version_checking.get_versioning_api = old_get_versioning_api
