...
from lightly_plus_time.lightly.transforms.sigToTensor import ToTensor
...
class BaseCollateFunction(nn.Module):
    ...
    def forward(self, batch: List[tuple]):
        ...
        transforms = (
            torch.cat(transforms[:batch_size], 0).reshape(batch_size, transforms[0].shape[0], transforms[0].shape[1]),
            torch.cat(transforms[batch_size:], 0).reshape(batch_size, transforms[0].shape[0], transforms[0].shape[1])
        )
...
class TSCollateFunction(BaseCollateFunction):
    ...
    def __init__(self,
        ...
        transform = [
            RandomSignalDrop(prob=0.5),
            Flip(prob=0.5),
            TimeShift(prob=0.3),
            AmplitudeShift(prob=0.3),
